...
if depth == depth_limit || all(Y==mean(Y)) || numel(Y) <= 1 || numel(colidx) == 0
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if numel(Y) == 0
        ...
        global K
        node.value = zeros(1,K);
        for i=1:K
            node.value(i) = (sum(Y==i)/size(Y,1));
        end
    end
    node.left = []; node.right = [];

    fprintf('depth %d [%d/%d/%d/%d/%d/%d/%d/%d/%d/%d]: Leaf node: = %s\n', depth, sum(Y==1), sum(Y==2), sum(Y==3), sum(Y==4), sum(Y==5), sum(Y==6), sum(Y==7), sum(Y==8), sum(Y==9), sum(Y==0), ...
        mat2str(node.value));
    return;
end
...
assert(isequal(unique(Z), [0; 1]), 'Z must be 1''s and 0''s.');
